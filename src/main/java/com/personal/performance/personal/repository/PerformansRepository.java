...
public interface PerformansRepository extends JpaRepository<PerformansEntity, Long> {
	...
	@Query("SELECT p FROM PerformansEntity p WHERE p.ekipId =:ekip and (p.haftaSira =:hafta1 or p.haftaSira =:hafta2)")
	...
	@Query("SELECT p FROM PerformansEntity p WHERE p.personelId =:personelId and p.haftaSira BETWEEN :hafta1 AND :hafta2")
    List<PerformansEntity> findByPerformansByHaftaSiraAndPersonelIdBtw(@Param("hafta1") Integer hafta1, @Param("hafta2") Integer hafta2, @Param("personelId") Integer personelId);
